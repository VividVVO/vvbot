...
// 加入公会 当前用户
func JoinClan(r *ghttp.Request, qqid int, groupName string) error {
	...
	/*
		isToJoinclan, err := clan_member.IsToJoinClan(qqid, )
		if err != nil {
			return errors.New(fmt.Sprintf("内部错误"))
		}
		if isToJoinclan {
			return errors.New(fmt.Sprintf("成员已加入公会"))
		}
	*/
...
// 获取公会成员信息
func GetUserClanMemberData(r *ghttp.Request, qqid int) (*clan_member.Entity, error) {
	if qqid == 0 {
		qqid = user.GetLoginData2(r).Qqid
	}
...
func GetClanDataAtQqid(r *ghttp.Request, qqid int) (*clan_group.Entity, error) {
	if qqid == 0 {
		qqid = user.GetLoginData2(r).Qqid
	}

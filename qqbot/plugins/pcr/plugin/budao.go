...
func (l Budao) Run(mess plugins.MeassageData, cm string, atqq int64) {
	...
	if len(cms) < 4 {
		bot.Send(mess.FromGroupID, mess.SendToType, "补刀格式应为：”[补刀|补尾刀|补掉刀] 伤害 周目 Boss编号 [昨日] [@某人] [:留言]“ 例：“补刀 100w 1 5 :一周目五王的刀”")
		return
	}
	var err error
	...
	if num >= 3 {
		if isYesterday {
			bot.Send(mess.FromGroupID, mess.SendToType, fmt.Sprintf("今日 %s 的上报次数已达到3次", clanMember.GameName))
		} else {
			bot.Send(mess.FromGroupID, mess.SendToType, fmt.Sprintf("昨日 %s 的上报次数已达到3次", clanMember.GameName))
		}
		return
	}

...
func (l Shenqchudao) Run(mess plugins.MeassageData, cm string, atqq int64) {
	...
	// 跨日，出刀申请自动清除
	if time2.GetPcrDayStartTimeToUnix(clanGroup.GameServer) > gvgGroup.ChallengeStratTime {
		gvgGroup.ChallengeStratQqid = 0
	}
	if gvgGroup.ChallengeStratQqid == qqid {
		bot.Send(mess.FromGroupID, mess.SendToType, fmt.Sprintf("%s 您正在挑战BOSS", clanMember.GameName))
		return
	}
	// 获取今日刀，过滤尾刀
	...
	if num >= 3 {
		bot.Send(mess.FromGroupID, mess.SendToType, fmt.Sprintf("今日 %s 的出刀次数已达到3次", clanMember.GameName))
		return
	}
	...
	if gvgGroup.BossLockType == 1 {
		...
		t := time.Now().Unix() - gvgGroup.BossLockTime
	...
	if gvgGroup.ChallengeStratQqid != 0 {
		...
		bot.Send(mess.FromGroupID, mess.SendToType, fmt.Sprintf("申请失败，%s 正在挑战boss", gameName))
		return
	}
